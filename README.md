...
| 路径分类 | 区分代理、直连和本地流量 |
| 热点与网络记账 | 按 Wi-Fi/热点分别记录，并显示本次热点中这台 Mac 的外网使用量 |
...
流量管家直接读取 macOS 提供的本机网络统计，不会上传流量记录，也不会安装系统扩展。应用会用物理网卡增量校准总量；无法准确归属到具体进程的协议开销分别显示为“未归属外网”或“未归属本地通信”。由于统计口径不同，结果仍可能与运营商账单或路由器数据存在少量差异。

网络记账使用本机生成的匿名网络标识，不保存 Wi-Fi 名称。“本次热点”只统计这台 Mac 的外网流量，不包含手机自身或连接到同一热点的其他设备。
...
- **本地**：当前 Mac 内部或同一局域网中的通信，不计入顶部外网总量，通常不会消耗宽带或手机套餐。

0.5.0 沿用 0.3.0 以来的日汇总数据，不会因升级再次清零。网络与热点会话从首次运行 0.5.0 后开始记录，不能追溯此前的网络使用情况。
